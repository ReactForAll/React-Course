...
In CRA you can make your PWA using the `workbox` libraries. Workbox is a set of libraries that helps you build PWAs. You can install the libraries using the following command
...
Now that you've installed the libraries, you can head to your `src` directory and create a new `service-worker.ts` file with the following code:
...
Next you'll need to register the service worker in your App. So let's create a `serviceWorkerRegistration.ts` which can take care of everything that you need to do for registering your service worker. You can create the `serviceWorkerRegistration.ts` in your `src` directory with the following:
...
Now you can register the service worker in your App's `index.tsx` file:
